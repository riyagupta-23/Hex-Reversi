...
/**
 * Represents the basic view of the reversi game with a square grid.
 */
public class ReversiSquareView extends JFrame implements ReversiView {
   ...
   */
  public ReversiSquareView(ReadOnlyIGame model) {
    super();
    ...
    // Use a BorderLayout with drawing panel in the center and button panel in the south
    this.setLayout(new BorderLayout());

    squareGrid = new SquareGrid();
    squareGrid.setBoardSideLength(model.cloneGame().getGrid().getLength());
    squareGrid.updateSquares(model.cloneGame().getGrid().getBoard());
    squareGrid.setPreferredSize(new Dimension(
            boardSideLength * 35, boardSideLength * 50));
    scrollPane = new JScrollPane(squareGrid);
    this.add(scrollPane, BorderLayout.CENTER);
    squareGrid.setOpaque(true);
    squareGrid.setBackground(Color.DARK_GRAY);

    // adding hint decorator
    HintDecorator hintDecorator = new HintDecorator();
    squareGrid.addDecorator(hintDecorator);
  ...
  private void initHintButtons() {
    ...
    enableHintsButton.addActionListener(e -> {
      squareGrid.setShowHints(true); // Enable showing hints
      this.refresh();
    ...
    disableHintsButton.addActionListener(e -> {
      squareGrid.setShowHints(false); // Disable showing hints
      this.refresh();
  ...
  private void mouseListener(ReadOnlyIGame model) {
    squareGrid.addMouseListener(new MouseListener() {
      @Override
      ...
      public void mousePressed(MouseEvent e) {
        lastHighlightedRow = squareGrid.getX(e.getX(), e.getY());
        lastHighlightedCol = squareGrid.getY(e.getX(), e.getY());
        squareGrid.highlight(e.getX(), e.getY());
        int hint = model.potentialFlips(squareGrid.getX(e.getX(), e.getY()),
                squareGrid.getY(e.getX(), e.getY()), model.getCurrentPlayer().getColor());
        squareGrid.showInfoOrHint(e.getX(), e.getY(), hint);
      }
  ...
  public void updateState(HashMap<CoordinateSystem, CellColor> updatedState) {
    squareGrid.updateSquares(updatedState);
  }
  ...
  public void showAutoPass() {
    String message = "No valid move available. Your turn has been passed.";
    JOptionPane.showMessageDialog(this, message,
